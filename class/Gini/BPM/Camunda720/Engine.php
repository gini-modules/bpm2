...
class Engine implements \Gini\BPM\Driver\Engine {
    ...
    public function __construct($config) {
        ...
        $url = $this->root. $options['api_list']['VerifyUser'];
        $response = $this->http
	    ->enableCookie()
        ...
        $this->userId = $rdata['authenticatedUser'];
        $this->authenticated = $rdata['authenticated'];
    }
    ...
    public function post($path, array $data=[]) {
        $response = $this->http
            ->header('Content-Type', 'application/json')
            ->post("{$this->root}/engine/{$this->engine}/$path", $data);
    ...
    public function get($path, array $data=[]) {
        $response = $this->http
            ->get("{$this->root}/engine/{$this->engine}/$path", $data);
        $status = $response->status();
        $data = json_decode($response->body, true);
        if (floor($status->code/100) != 2) {
            throw new \Gini\BPM\Exception($status->code . ': '. $data['message']);
    ...
    public function delete($path, array $data=[]) {
        $response = $this->http
            ->header('Content-Type', 'application/json')
            ->delete("{$this->root}/engine/{$this->engine}/$path", $data);
        $status = $response->status();
        $data = json_decode($response->body, true);
        if (floor($status->code/100) != 2) {
            throw new \Gini\BPM\Exception($status->code . ': '. $data['message']);
    ...
    public function put($path, array $data=[]) {
        $response = $this->http
            ->header('Content-Type', 'application/json')
            ->put("{$this->root}/engine/{$this->engine}/$path", $data);
        $status = $response->status();
        $data = json_decode($response->body, true);
        if (floor($status->code/100) != 2) {
            throw new \Gini\BPM\Exception($status->code . ': '. $data['message']);
    ...
    public function deploy($name, $files) {
        ...
        if(is_array($files[0]) && $files[0]['content']) {
            ...
            $response = $this->http
                ...
                ->post("$root/engine/$engine/deployment/create", $multiPartData);
        } else { // 文件
            foreach ($files as $file) {
            ...
            $response = $this->http
                ...
                ->post("$root/engine/$engine/deployment/create", $data);
        }
